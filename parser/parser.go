...
type (
	...
	infixParseFn  func(statement ast.Expression) ast.Expression
)
...
// parseExpression finds an associated prefixParseFn with the current token and calls it.
// Then tries to find infixParseFns for the next token, until it encounters a lower precedence token.
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	// While not at the end of the hash...
	for !p.peekTokenIs(token.RBRACE) {
		// go to the next token
		p.nextToken()
		key := p.parseExpression(LOWEST)

		// if not a colon after key return nil
		...
		// next token should either be a rbrace or comma
		if !p.peekTokenIs(token.RBRACE) && !p.expectPeek(token.COMMA) {
			return nil
		}
	}
	// once finished, the final token should be a rbrace
