...
`,
	...
	for _, tt := range tests {
		...
		if errorObj.Message != tt.expectedMessage {
			t.Errorf("expected message does not match returned error message. expected=%q, got=%q", tt.expectedMessage, errorObj.Message)
		}
	}
}
...
func testBooleanObject(t *testing.T, evaluated object.Object, expected bool) bool {
	...
	if !ok {
		t.Errorf("object is not boolean type,got=%T (%+v)", evaluated, evaluated)
		return false
	}
...
func testIntegerObject(t *testing.T, evaluated object.Object, expected int64) bool {
	...
	if result.Value != expected {
		t.Errorf("result has wrong value, got=%d, want=%d", result.Value, expected)
		return false
	}
